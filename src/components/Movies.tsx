  ...
  if (selected) {
    ...
    return (
      <div className="flex h-full bg-gray-950">
        ...
        <div className="w-80 flex-shrink-0 relative bg-black">
          {posterUrl ? (
          ...
          {/* Note + favori sur l'affiche */}
          <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-black/70 rounded-xl px-2.5 py-1.5">
          ...
          <button
            onClick={handleFav}
            className="absolute top-3 right-3 w-9 h-9 rounded-full bg-black/70 flex items-center justify-center transition hover:bg-black/90"
          >
            <svg className={`w-5 h-5 ${isFav ? 'text-red-500' : 'text-white'}`} viewBox="0 0 24 24" fill={isFav ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
          ...
          <button
            ...
            className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/70 hover:bg-black/90 text-white text-sm px-4 py-2 rounded-xl transition"
          >
        ...
        {/* Colonne droite : infos + player */}
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* En-tête infos */}
          <div className="px-6 pt-5 pb-3 flex-shrink-0">
            <h1 className="text-white font-bold text-2xl leading-tight mb-1">{selected.name}</h1>
            {selected.genre && <p className="text-gray-400 text-sm mb-2">{selected.genre}</p>}
            <div className="flex items-center gap-3">
              {/* Étoiles */}
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg key={i} className={`w-4 h-4 ${i < rating ? 'text-yellow-400' : 'text-gray-700'}`} viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 2l3.09 6.26L22 9.27l-5 4.87L18.18 21 12 17.77 5.82 21 7 14.14l-5-4.87 6.91-1.01L12 2z"/>
                  </svg>
                ))}
              </div>
              {selected.release_date && (
                <span className="text-gray-400 text-sm">Release Date : {selected.release_date}</span>
              )}
            </div>
          </div>

          {/* Player */}
          <div className="flex-shrink-0 relative bg-black mx-6 rounded-2xl overflow-hidden" style={{ aspectRatio: '16/9' }}>
            {playing ? (
              <>
              ...
              <div className="w-full h-full flex items-center justify-center">
                {posterUrl && (
                  <img src={posterUrl} alt="" className="w-full h-full object-cover opacity-30 absolute inset-0" />
                )}
                <button
                  onClick={handlePlay}
                  className="relative z-10 w-16 h-16 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur border border-white/20 flex items-center justify-center transition"
                >
